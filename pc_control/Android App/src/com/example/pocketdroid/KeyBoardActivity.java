...
public class KeyBoardActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        tvsel = (TextView)findViewById(R.id.tvSelCmd);
        lv = (ListView)findViewById(R.id.lvCommands);
        cdb = new CommandDB();
        readFromFile();
        if(cdb == null){
        	cdb = new CommandDB();
    ...
    void reFillList(){
    	...
    	for(int i=0;i<cdb.myList.size();i++){
    		...
    		d = "";
    		for(String s : cdb.myList.elementAt(i).desc){
    			d+=s + " + ";
    		}
    		if(d.contains(" + ")){
    	...
    	lv.setAdapter(new MyArrayAdapter(KeyBoardActivity.this, names, descs));
    }
    
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.activity_key_board, menu);
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case R.id.AddCommand:
        	myIntent = new Intent(this,AddCommandActivity.class);
        	startActivityForResult(myIntent, 1);
            return true;
        case R.id.RemoveCommand:
...
//        	return true;
    ...
    void writeToFile(){
		try{
			FileOutputStream fos = openFileOutput("cmd_file", Context.MODE_PRIVATE);
	...
	void readFromFile(){
		try{
			FileInputStream fin = openFileInput("cmd_file");
